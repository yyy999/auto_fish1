...
def get_depth(symbol, type):
    """
    :param symbol
    :param type: 可选值：{  step0, step1, step2, step3, step4, step5 }
    :return:
    """
    params = {'symbol': symbol,
              'type': type}
...
def orders_history():
    ...
    """
    params = {}
    url = "/v1/order/history"
    return api_key_get(params, url)
# 查询48H内订单的成交明细
def get_orderid(clientorderid):
    """
    :param order_id:
    :return:
    """
    params = {}
    url = "/v1/order/orders/{0}".format(str(clientorderid))
    return api_key_get(params, url)
# 查询当前委托、历史委托返回委托价
